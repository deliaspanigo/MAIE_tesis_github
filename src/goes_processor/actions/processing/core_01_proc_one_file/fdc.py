...
def fn01_process_visuals(scn, main_out_dir, base_name):
...
def fn02_process_analysis(scn, scn_res, main_out_dir, base_name):
...
def fn03_process_vectorial(scn, scn_res, main_out_dir, base_name):
    """FN03: Clustering and Vector export (GeoJSON, SHP, KMZ)."""
...
def process_fdc_single_file(input_file, input_base: Path, output_base: Path, overwrite=False, indent=""):
    """FDC Orchestrator v0.3.3 compatible with cli_core01.py"""
    start_ts = time.time()
    ...
    try:
        ...
        scn_res = fn01_process_visuals(scn, main_out_dir, base_name)
        report["fn01"] = True
        
        fn02_process_analysis(scn, scn_res, main_out_dir, base_name)
        report["fn02"] = True
        
        fn03_process_vectorial(scn, scn_res, main_out_dir, base_name)
        report["fn03"] = True
