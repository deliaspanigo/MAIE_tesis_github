...
# Searches using full regex (including bucket) from data_raw root.
#
# Features:
#   - If file_local["path_absolute"] exists → uses it directly
#   - If not → uses file_s3["regex"] to search from data_raw root
...
def check_local_file_existence(plan: dict | list[dict]) -> dict | list[dict]:
    ...
    # Base root: use ONLY get_my_path("data_raw") (no extra /goes_raw)
    ...
    if data_raw_root.exists():
        print(f"DEBUG: Subfolders in data_raw: {[p.name for p in data_raw_root.iterdir() if p.is_dir()]}")
    ...
    for single_plan in plans:
        ...
        for file_key, item in inventory.items():
            ...
            if abs_path_str:
                ...
                # Use full regex from data_raw root
                ...
                if not regex:
                    ...
                    if valid_candidates:
                        ...
                        # Take the most recent by modification time
            ...
            if found_path:
                ...
                item["file_local"]["file_name"] = found_path.name  # Nombre del archivo encontrado
                ...
                item["file_local"]["file_name"] = None
