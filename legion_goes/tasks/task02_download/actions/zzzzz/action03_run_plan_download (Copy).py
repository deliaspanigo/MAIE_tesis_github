...
# Version: 1.7.0 (English UI - Dual Path Ready)
# Description: Professional S3 downloader with real-time status table.
# =============================================================================
...
def download_s3_item_worker(fid, info, s3_client):
    ...
    try:
        # Get real name from Bulk Scan or look it up
        ...
        if not real_name:
            ...
            # Convert MB back to Bytes for validation
        ...
        if abs(local_size_bytes - s3_size_bytes) < (0.1 * 1024 * 1024): # Error margin for rounding
...
def update_inventory_item_with_s3_metadata(plan_data: dict, fid: str, s3_file_name: str, exists_online: bool, s3_file_size: any) -> dict:
    ...
    if fid in plan_data["download_inventory"]:
        ...
        item["file_s3"]["size_mb"] = s3_file_size
    
    return plan_data

# --- 4. DAILY REPORT (English Version) ---
...
def execute_task02_download_action03_run_download(path_plan: Path, threads=5, checkpoint_n=10):
    ...
    try:
        ...
        save_dict_plan_json(path_plan, plan_data)
        plan_data = update_dict_plan_check_all(plan_data)
        print(f"\n📊 Final Status: {plan_data['summary']['local_total_files']}/{total_planned} files ready.")
        display_daily_status_report(plan_data)
